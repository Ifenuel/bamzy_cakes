...
export async function getDashboard(_req, res) {
  try {
    ...
    const [ordersToday, revenueToday, pendingOrders, availableProducts, upcomingBookings, upcomingTrainings, recentOrders, customerCount, allTimeRevenue, allTimeOrders] = await Promise.all([
      ...
      pool.query("SELECT COALESCE(SUM(total), 0) as total FROM orders WHERE payment_status = 'successful'"),
    ...
    return success(res, {
      ...
      totalRevenue: parseFloat(allTimeRevenue.rows[0].total),
      totalOrders: parseInt(allTimeOrders.rows[0].count),
